...
void proc_freekpt(pagetable_t kernelpt);
...
found:
  ...
  if (!(p->alarm_trapframe = (struct trapframe*)kalloc())) 
  {
    freeproc(p);
    release(&p->lock);
    return 0;
...
void
proc_freekpt(pagetable_t kernelpt)
{
  for(int i = 0; i < 512; i++)
  {
    pte_t p = kernelpt[i];
    ...
    {
      kernelpt[i] = 0;
      if (!(p & (PTE_R | PTE_W | PTE_X)))
      {
        uint64 v = PTE2PA(p);
        proc_freekpt((pagetable_t)v);
      }
    }
  }
  kfree((void*)kernelpt);
